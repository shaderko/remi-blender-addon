...
from .workflow.disk_service import (
...
class RemiSessionRuntime:
    ...
    def object(self, context):
        ...
        for obj in bpy.data.objects:
            if obj.get(SESSION_ID_KEY) == state.session_id:
                return obj
        return None
    ...
    def _create_working_copy(self, context, source, label: str, *, linked=True):
        ...
        if linked:
            ...
            self._select_only(context, working)
        return working
    ...
    @staticmethod
    def _remove_new_orphans(before):
    ...
    @staticmethod
    def _remove_orphan_dependencies(dependencies):
    ...
    def _append_checkpoint_object(self, context, slot: str):
        ...
        dependencies_before = self._dependency_snapshot()
        materials_before = {material.name: material for material in bpy.data.materials}
        restored, manifest = self._require_disk().load(
            slot,
    ...
    def _load_checkpoint(self, context, slot: str):
        ...
        current_dependencies = self._object_dependencies(current)
        restored, dependencies_before, manifest = self._append_checkpoint_object(context, slot)
        ...
        self._remove_orphan_dependencies(current_dependencies)
        self._remove_new_orphans(dependencies_before)
        restored.name = name
        restored[SESSION_ID_KEY] = self.state(context).session_id
        self._select_only(context, restored)
        return restored

    def load_source_temporary(self, context):
        """Load the source checkpoint as a disposable object for texture baking."""
        restored, dependencies_before, _manifest = self._append_checkpoint_object(
            context,
            "source",
        )
    ...
    def cleanup_temporary_source(self, source, dependencies_before):
        ...
        try:
            ...
            exists = False
        if exists:
            source_dependencies = self._object_dependencies(source)
            self._remove_object(source)
            self._remove_orphan_dependencies(source_dependencies)
        self._remove_new_orphans(dependencies_before)
    ...
    def _commit_step(self, context, candidate, label: str, next_stage=None):
        ...
        old_dependencies = self._object_dependencies(current)
        ...
        self._remove_object(current)
        self._remove_orphan_dependencies(old_dependencies)
